...
namespace WebGamesCatalog.Repositories
{
    public class GameRepository : IGameRepository
    {
        ...
        Task.FromResult(games.Values.Skip((page - 1) * quantity).Take(quantity).ToList());
        ...
        Task.FromResult(games.Values.Where(game => game.Name.Equals(name) && game.Prod.Equals(prod)).ToList());
